...
- `download_scripts` stages each file with `install` (`root:root` when privileged) next to its destination and `mv`s it into place so a running `update_self.sh` keeps its inode and the replace stays same-filesystem
- `INSTALL_DIR` defaults to the directory `update_self.sh` runs from (`BASH_SOURCE`), never `$HOME` — under root cron `$HOME/pi-scripts` resolved to `/root/pi-scripts` and updated a copy cron never executed
...
- **Root-owned install tree**: `INSTALL_DIR` defaults to `/usr/local/lib/pi-maintenance`. Writes go through `sudo` when the tree is not writable (or directly when already root), and files land as `root:root` — `0755` directories/scripts, `0644` `lib/*.sh` and `.version`. Root cron executes these scripts, so a user-writable location would be a user→root escalation. A user-writable `INSTALL_DIR` override (tests) is written directly and left as-is. `INSTALL_DIR="$HOME/pi-scripts"` (exported by `update_self.sh` ≤ v1.1.4) is redirected to the default
- **Legacy migration**: `_legacy_install_dirs` lists `$LEGACY_INSTALL_DIR` (default `$HOME/pi-scripts`) plus any directory a suite crontab line still points at; both `--update` and the interactive entry re-download into `$INSTALL_DIR`, rewrite root + user crontab paths (`_repoint_crontab_dir`), then delete legacy directories that contain **only** suite files (`.version`, a `lib/` holding only the three helper libs, the seven scripts, staging leftovers — `_legacy_dir_is_suite_only`). Anything else (e.g. a home directory referenced by a manual crontab line) is left in place with a notice. Retirement happens only after `download_scripts` succeeded (it returns non-zero when the tree cannot be created or any file failed) and after `_repoint_crontab_dir` verified no crontab still references the legacy path; otherwise crontabs and legacy files are left untouched and `--update` exits non-zero "Already installed" = `$INSTALL_DIR` exists **or** a legacy tree is present
